...
public class BinarySearchTree {
	...
	public TreeNode delete(TreeNode root, int val) {
		if (root.data > val) {  // if val is less than root data
			...
			root.right = delete(root.right, is.data);//recursively call the delete method where root will be right node and val is inorder successor
		}
		return root;
	}
	...
	public boolean isValid(TreeNode root, long min, long max) {
		...
		if (left) { //if left recursion returns true
			boolean right = isValid(root.right, root.data, max); // recursive call in right subtree where min is root data
			return right;
		}
		return false;
	}
...
//convert sorted array into binary search tree
	//given an array where elements are stored in ascending order, convert it into a height 
	//balance BST
	//height balance tree is defined as a binary tree in which the depth of the two subtrees
	//of every node never differ by more than 1
//thought before coding
	// we are given an sorted array, which number we choose to be a root of current subtree
